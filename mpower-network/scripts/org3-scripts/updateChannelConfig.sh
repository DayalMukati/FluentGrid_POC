...
set -x
jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"Org3MSP":.[1]}}}}}' config.json ./organizations/peerOrganizations/org3.mpower.in/org3.json > modified_config.json
{ set +x; } 2>/dev/null
...
setGlobals 2
set -x
peer channel update -f org3_update_in_envelope.pb -c ${CHANNEL_NAME} -o orderer.mpower.in:7050 --ordererTLSHostnameOverride orderer.mpower.in --tls --cafile "$ORDERER_CA"
{ set +x; } 2>/dev/null
